...
<main>
  ...
  <div class="container marketing" style="margin-top: 20px">
    ...
    <div class="row">
      ...
      <center>
        <div class="col-lg-6">
          <form method="post" action="pontuar_participante_efetua.php">
            <br></br>
            <label>Nome:</label>  
              <select name="nome" style="width: 100%;" required>
                  <option></option>
                  <?php 
                      $dados_cab2 = "SELECT * FROM participantes WHERE ativo_participante = '1' ORDER BY nome_participante";
                      $result_dados2 = mysqli_query($conn, $dados_cab2);

                      while ($row_dados2 = mysqli_fetch_assoc($result_dados2)) { ?>
                          
                          <option value=<?php echo $row_dados2['id_participante']; ?>><?php echo $row_dados2['nome_participante']." -- ".$row_dados2['pontuacao_participante'] ; ?></option>
                  <?php
                      }
                  ?>
              </select><br></br>
            ...
            <input type="submit" name="botao" class="btn btn-success" value="Pontuar">
          </form>
        </div><br></br>
      </center>
      ...
      <?php 
                      $dados_cab2 = "SELECT * FROM participantes WHERE ativo_participante = '1'";
                      $result_dados2 = mysqli_query($conn, $dados_cab2);

                      while ($row_dados2 = mysqli_fetch_assoc($result_dados2)) { ?>
                          
                          <div class="col-lg-6">
                           <b>Nome: </b><?php echo $row_dados2['nome_participante']; ?>
                          </div>
                          <div class="col-lg-6">
      ...
      <h5>Geração Eleita</h5>
      <?php
            $pontos_a = 0;

            $dados_cab3 = "SELECT * FROM participantes WHERE ativo_participante = '1' AND id_equipe_participante = 1";
                      $result_dados3 = mysqli_query($conn, $dados_cab3);

                      while ($row_dados3 = mysqli_fetch_assoc($result_dados3)) {

                          $pontos_a = $pontos_a + $row_dados3['pontuacao_participante'];

                      }
            
            echo "<p>Pontuação: $pontos_a</p>";
      ?>

      <h5>Unidos em Cristo</h5>
      <?php
            $pontos_b = 0;

            $dados_cab4 = "SELECT * FROM participantes WHERE ativo_participante = '1' AND id_equipe_participante = 2";
                      $result_dados4 = mysqli_query($conn, $dados_cab4);

                      while ($row_dados4 = mysqli_fetch_assoc($result_dados4)) {

                          $pontos_b = $pontos_b + $row_dados4['pontuacao_participante'];

                      }
            
            echo "<p>Pontuação: $pontos_b</p>";
      ?>
